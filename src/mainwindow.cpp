...
void MainWindow::refreshEventsTab(Event::Group eventGroup) {
    ...
    const struct EventTabUI tabUI = groupToUI.value(eventGroup); // TODO: Get pointer to UI struct instead?
    ...
    // Create the event frames for the selected events. If they've already been created we just repopulate it.
    QList<QFrame *> frames;
    ...
    if (tabUI.contents && tabUI.contents->layout() && tabUI.contents->children().length()) {
...
// TODO: Redisplaying an events tab squishes all the frames
